...
class OllamaLLMClient:
    ...
    def _build_analysis_prompt(self, ocr_text: str, grade: str = '') -> str:
        ...
        return f"""你是一个小学错题分析老师，专门帮助小学生理解和纠正错题。请仔细观察图片中的题目，返回JSON格式分析结果：
...
3. **红色批改笔迹（老师/家长订正）**：
   - 用红色笔写的批改，通常包括：打勾（√）、打叉（×）、圈出错误、写上正确答案
   - **红色笔迹是批改标记，不是学生的答案**
...
{{
  ...
  "question_text": "题目完整内容（只包含印刷体，括号和横线保持原样）",
...
2. **学生答案识别要求**：
   ...
   - **区分笔迹颜色**：黑色/蓝色手写体 = 学生的答案，红色手写体 = 老师的批改订正
...
3. **正确答案推理要求**：
   ...
   - 对于几何题，要根据几何定义和性质推理
   - **最终验证步骤（非常重要）**：检查 student_answer 和 correct_answer 是否相同，如果相同，student_answer 必须置空

4. **目标学生群体**：这是给小学生看的（1-6年级），请用简单易懂的语言
...
6. **error_type 分类标准**（必须从以下5个中选择一个，只能返回中文）**：
